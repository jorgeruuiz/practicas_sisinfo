...
import { getSocket, connect, raw as rawSocket, disconnect } from './socketClient'
...
export function SocketProvider({ children }) {
  ...
  const api = {
    raw: () => getSocket(),
    on: (ev, cb) => { const s = ensureSocketRef(); if (s) s.on(ev, cb) },
    off: (ev, cb) => { const s = socketRef.current || getSocket(); if (s) s.off(ev, cb) },
    emit: (ev, payload) => { const s = socketRef.current || getSocket(); if (s) s.emit(ev, payload) },
    connected
  }
