...
import {
  httpRespuestaWebPositiva,
  httpRespuestaWebNegativa,
} from "./htmlEnviables.js";
...
export async function crearUsuario(req, res) {
  try {
    ...
    res.json({
      message:
        "Registro exitoso. Revisa tu correo para verificar tu cuenta. ¡Ten cuidado!, el correo ha podido ser clasificado como spam.",
    });
  } catch (error) {
...
export async function resendVerificationEmail(req, res) {
  try {
    ...
    await db
      .update(usuario)
      .set({ tokenVerificacion: token })
      .where(eq(usuario.Correo, Correo));
    // Enviar correo de verificación
    await sendVerificationEmail(Correo, token);
    res.json({
      message:
        "Correo de verificación reenviado. ¡Ten cuidado!, el correo ha podido ser clasificado como spam.",
    });
  } catch (error) {
    res
      .status(500)
...
export async function authenticate(socket) {
  ...
  try {
    const token = socket.handshake.query.token;
    if (!token) {
      console.error("Sin token, desconectando...");
      socket.disconnect();
      return;
    }

    // ¡Usa verify para confiar en el payload!
    const verified = jwt.verify(token, process.env.JWT_SECRET);
    const userId = verified.userId;

    // IMPORTANTE: guarda el usuario en el socket
    socket.user = { id: userId }; // <-- ESTA LÍNEA ES LA CLAVE

    // Si quieres, puedes enriquecerlo con datos de BD:
    // const [u] = await db.select().from(usuario).where(eq(usuario.id, userId));
    // if (u) socket.user.NombreUser = u.NombreUser;

    // Manejo de sesión única (opcional)
    if (activeSockets.has(userId)) {
      const existingSocket = activeSockets.get(userId);
      existingSocket.disconnect();
    }
...
export async function editUser(req, res) {
  try {
    ...
    if (FotoPerfil.length < 4) {
      res
        .status(400)
        .json({ error: "La foto de perfil debe tener al menos 4 caracteres" });
      return;
    }
    ...
    if (user.estadoUser !== "logged") {
      res.status(400).json({
        error: "Usuario no logueado. Inicie sesión para editar su perfil",
      });
      return;
    }
    if (user.correoVerificado === "no") {
      res.status(400).json({
        error:
          "Correo no verificado. Por favor, verifica tu correo antes de editar tu perfil",
      });
      return;
    }
...
export async function sendPasswdReset(req, res) {
  try {
    ...
    if (user.correoVerificado == "no") {
      res.status(400).json({
        error:
          "El usuario debe haber verificado su correo para restablecer la contraseña.",
      });
      return;
    }
    ...
    await db
      .update(usuario)
      .set({ tokenPasswd: hashedToken })
      .where(eq(usuario.Correo, Correo));
    await sendChangePasswdEmail(Correo, token);
    res.json({
      message:
        "Correo de restablecimiento de contraseña enviado. ¡Ten cuidado!, el correo ha podido ser clasificado como spam.",
    });
  } catch (error) {
    res.status(500).json({
...
export async function resetPasswd(req, res) {
  try {
    ...
    const usuarios = await db
      .select()
      .from(usuario)
      .where(eq(usuario.NombreUser, user));
    if (usuarios.length === 0) {
      res.status(400).json({ error: "Correo no registrado" });
      return;
    }
    ...
    if (!isMatch) {
      res.status(400).json({
        error: "Token incorrecto, no se reestablecerá la contraseña.",
      });
      return;
    }
    ...
    await db
      ...
      .where(eq(usuario.id, userAux.id));
